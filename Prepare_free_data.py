...
if not path.exists(dta1_name):
    ...
    question_dta = dta[
        ['asker_uid', 'question_all', 'time', 'author_name', 'look_price', 'onlooker_count', 'ask_price', 'label', 'oid']]
    ...
    filesovler.write_pkl(dta1_name, question_dta)
...
if not path.exists(dta2_name):
    ...
    print(question_dta.info())
    question_dta = question_dta[question_dta['look_price'] == 0]
    free_dta = filesovler.read_pkl('../qa_txt/question191227.pkl')
    free_dta_lst = free_dta.set_index('oid')['onlooker_count'].to_dict()
    question_dta = question_dta.reset_index(drop=True)
    l_c = []
    for idx in question_dta.index:
        try:
            f_c = int(free_dta_lst[question_dta['oid'].iloc[idx]])
            l_c.append(f_c - int(question_dta['onlooker_count'].iloc[idx]))
        except KeyError:
            print(idx)
            l_c.append(np.nan)
    question_dta['onlooker_count'] = l_c
    question_dta = question_dta.dropna()
    question_dta = question_dta.reset_index(drop=True)
    print(len(question_dta))
    filesovler.write_pkl(dta2_name, question_dta)
...
if not path.exists(dta3_name):
    ...
    filesovler.write_pkl(dta3_name, question_dta)
...
if not path.exists(dta5_name):
    ...
    # bin_file = '/Users/wisdombeat/PycharmProjects/qa_txt/cc.zh.300.bin'
    # model = fasttext.load_model(bin_file)
    # uni_word = filesovler.read_pkl('../qa_txt/unique_word.pkl')
    word_vector = filesovler.read_pkl('../qa_txt/word_vector.pkl')
    word_norm = filesovler.read_pkl('../qa_txt/word_norm.pkl')
    # for question in question_dta['question_context']:
    #     for word in question_dta:
    ...
    #             word_norm[word] = np.linalg.norm(word_vector[word])
    #         except KeyError:
    #             print(word)
    # filesovler.write_pkl('../qa_txt/umique_word.pkl', word)
    # filesovler.write_pkl('../qa_txt/word_vector.pkl', word_vector)
    # filesovler.write_pkl('../qa_txt/word_norm.pkl', word_norm)
    print('模型加载完毕')
...
if not path.exists(dta6_name):
    ...
    for idx in question_dta.index:
        try:
            ...
            question_dta['readability'].iloc[idx] = np.nan
    question_dta.dropna(inplace=True)
    question_dta = question_dta.reset_index(drop=True)
